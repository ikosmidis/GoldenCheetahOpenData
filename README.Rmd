...
We can also compute and visualize the summaries for the workout sessions (see, [trackeR's vignette](https://cran.r-project.org/package=trackeR/vignettes/trackeR.pdf) for details)
...
A bit more advanced analytics: The power concentration profiles for the this athlete ID are
```{r trackeRdata3, fig.height = 7}
athlete1_cp <- concentration_profile(athlete1, what = c("power"))
plot(athlete1_cp, multiple = TRUE)
```
and a functional PCA on them gives that the first 4 components explain about 95% of the variability in the concentration profiles
```{r}
